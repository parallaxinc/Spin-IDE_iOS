...
- (int) find: (CodeView *) text {
    if (findString != nil && findString.length > 0) {
        ...
        if (backwardSearch) {
            startSearchRange.length = startSearchRange.location;
        ...
        if (wrap && ((!backwardSearch && startSearchRange.location > 0) || backwardSearch)) {
            if (backwardSearch) {
                if (startSearchRange.location > [[text text] length])
                    searchRange.location = 0;
                else
                    searchRange.location += startSearchRange.length;
                searchRange.length = [[text text] length] - searchRange.location;
            } else {
                searchRange.location = 0;
